...
import { supabase, initializeDatabase, type SupabaseSubmission } from "./supabase";
...
class RealtimeDataStore {
  ...
  private async initialize() {
    try {
      ...
      console.error('Failed to initialize real-time store, using fallback:', error);
      this.loadFromFallback();
      this.isInitialized = true;
    }
  }
  ...
  private mapToSupabase(submission: Submission): Omit<SupabaseSubmission, 'created_at' | 'updated_at'> {
    return {
      id: submission.id,
